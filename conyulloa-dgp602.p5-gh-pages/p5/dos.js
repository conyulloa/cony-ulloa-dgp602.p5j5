...
var porcentaje = [49.4,48.9,48.6,48.3,48.6,48.3,48.5,47.9,48.3]
...
function draw() {
  ...
  for(var x = 0; x <= mujeres.length; x++){
    ...
    textSize((porcentaje[x]*3));
    // aquí dame el nro. de oscar ganado y ubícalo en una parte del ancho del canvas
    text(porcentaje[x], ((width/(mujeres.length+1))*(x+1)), (height/2));
    //re-defino el blanco
    fill(255);
    ...
    text(mujeres[x], ((width/(mujeres.length+1))*(x+1)), ((height/2)+12),((windowWidth/(mujeres.length+1))-50),100);
  }
}
